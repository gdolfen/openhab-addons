...
/**
 * Handles Key Control Command Right.
 *
 * @author Guido Dolfen
 */
@NonNullByDefault
public class KeyControlRight extends BaseChannelHandler<Void, Object> {
    ...
    public void onReceiveCommand(@Nullable ConnectableDevice device, String channelId, LGWebOSHandler handler,
        ...
        if (device.hasCapabilities(KeyControl.Right)) {
            getControl(device).right(getDefaultResponseListener());
        }
    }
}
