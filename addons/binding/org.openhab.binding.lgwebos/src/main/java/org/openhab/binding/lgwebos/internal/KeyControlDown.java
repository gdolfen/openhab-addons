...
/**
 * Handles Key Control Command Down.
 *
 * @author Guido Dolfen
 */
@NonNullByDefault
public class KeyControlDown extends BaseChannelHandler<Void, Object> {
    ...
    public void onReceiveCommand(@Nullable ConnectableDevice device, String channelId, LGWebOSHandler handler,
        ...
        if (device.hasCapabilities(KeyControl.Down)) {
            getControl(device).down(getDefaultResponseListener());
        }
    }
}
