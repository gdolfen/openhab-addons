...
/**
 * Handles Key Control Command Back.
 *
 * @author Guido Dolfen
 */
@NonNullByDefault
public class KeyControlBack extends BaseChannelHandler<Void, Object> {
    ...
    public void onReceiveCommand(@Nullable ConnectableDevice device, String channelId, LGWebOSHandler handler,
        ...
        if (device.hasCapabilities(KeyControl.Back)) {
            getControl(device).back(getDefaultResponseListener());
        }
    }
}
